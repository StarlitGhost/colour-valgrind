...
r"                           (?: \s+ (?&_CV_QUALIFIER_) )? )"
r"  (?P<_TYPE_>             (?: (?&_QUALIFIED_TYPE_) \s* (?&_OPERATOR_){0,2} ) )"
...
r"                          ) )"
r"  (?P<_CAST_>             (?: \( (?&_TYPE_) \) ) )"
r"  (?P<_TEMPLATE_ARG_>     (?: (?&_PARAM_) | (?&_CAST_)? (?&_NUMERIC_LITERAL_) ) )"
...
r"  (?: (?&_TEMPLATE_) )?"
r"  \s*"
r"  \(\s* (?P<PARAMETER_TYPE_LIST> (?&_PARAM_LIST_) (?: \s* , \s* \.\.\. )? )? \s*\)"
r"  (?:"
...
r"  )?"
r"  (?:"
r"   \(\s* (?P<PARAMETER_TYPE_LIST> (?&_TYPE_LIST_) )? \s*\)"
r"  )?"
r" )"
+ cpp_grammar, re.VERBOSE)
